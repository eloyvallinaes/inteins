...
class MeasureInterpro:
    ...
    def physcoprops(cls, seq, acc):
        # aa composition
        L = len(seq)
        props = {f"{letter}": seq.count(letter) / L for letter in MeasureInterpro.AA}
        # accession
        props['refseq_accno'] = acc
        # properties
        fPos = sum([seq.count(letter) for letter in "KR"]) / L
        ...
        props.update({
            f"{key}": val
            for key, val in zip(MeasureInterpro.COLNAMES, values)
        })
        return props
    ...
    def measure_inteins(self):
        ...
        for acc, regions in self.regions.items():
            for intein in regions['inteins']:
                data.append(MeasureInterpro.physcoprops(intein['seq'], acc))
        return data

    def measure_host(self):
        data = []
        for acc, regions in self.regions.items():
            data.append(MeasureInterpro.physcoprops(regions['host'], acc))
        return data
